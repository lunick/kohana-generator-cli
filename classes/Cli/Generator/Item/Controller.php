 ...
 *
 */
class Cli_Generator_Item_Controller extends Cli_Generator_Abstract_Item_Controller {
    ...
    public function init(){
        ...
        $this->setup(Cli_Util_System::$CONTROLLER)
                    ->add_row("class Controller_" . $class_name . " extends Controller {")
                    ->add_row();
        ...
        {    
            foreach ($this->get_actions() as $action){
                $this->add_row("/**", 4)
                        ...
                        ->add_row("public function action_".$action."()", 4)
                        ->add_row("{", 4)
                        ->add_row("\$this->response->body('<h1>".$action."</h1>');", 8)
                        ->add_row("}", 4)
                        ->add_row();
            }
        }
    ...
    public static function factory($filename = null, $table = null){
        return new Cli_Generator_Item_Controller($filename, $table);
    }
