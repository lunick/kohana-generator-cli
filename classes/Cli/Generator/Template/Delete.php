 ...
 *
 */
final class Cli_Generator_Template_Delete extends Cli_Generator_Abstract_Template {
    
    public function prompt(\Cli_Generator_Interface_InteractivePrompt $prompt, \Cli_Generator_Interface_Options $options) {
        $prompt->add_prompt("all", lang(array("all_delete", "yn", "skip")), "yes|no|y|n", false)
        ...
        if($orm->table_exists()){
            ...
            {            
                ...
                $writer->add_row()
                       ...
                       ->add_row("<?php echo Form::hidden('id', \$model->id); ?>", 8)
                       ->add_row("<?php echo Form::submit('submit', __('action.delete'), array('class' => 'btn btn-danger')); ?>", 8)
