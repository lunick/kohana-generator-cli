 ...
 *
 */
class Generator_Item_List extends Generator_Abstract_Item {
    ...
    public function init() {
        ...
        foreach ($fields as $field) {
            $this->get_file_builder()
                    ->add_row("<th><?php echo __('" . $name . "." . $field->get_name() . "'); ?></th>", 12);
        }
            
        $this->get_file_builder()
                ->add_row("<th><?php echo __('action.actions'); ?></th>", 12)
                ->add_row("</tr>", 8)
                ->add_row("</thead>", 4)
                ->add_row("<tfoot>", 4)
                ->add_row("<tr>", 8);
        ...
        $this->get_file_builder()
                ->add_row("<td><?php echo HTML::anchor(\$route.'/new', __('action.create_new')); ?></td>", 12)
                ->add_row("</tr>", 8)
                ->add_row("</tfoot>", 4)
                ->add_row("<tbody>", 4)
        ...
        foreach ($fields as $field) {
            $this->get_file_builder()
                    ->add_row("<td><?php echo HTML::chars(\$item->" . $field->get_name() . "); ?></td>", 12);
        }
